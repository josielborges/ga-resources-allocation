<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h2 class="text-xl font-semibold text-text-primary">Tribos</h2>
    </div>

    <div class="bg-white rounded-md shadow-sm overflow-hidden">
      <div v-if="loading" class="p-8 text-center">
        <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-main mx-auto"></div>
        <p class="mt-2 text-text-secondary">Carregando tribos...</p>
      </div>
      ...
      <table v-else class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="tribo in tribosOrdenadas" :key="tribo.id" class="hover:bg-gray-50">
            <td class="px-4 py-2">
              <input 
                v-if="editando === tribo.id"
                v-model="nomeEditando"
                @keyup.enter="salvarEdicao(tribo)"
                ...
                ref="inputEdit"
              />
              <div v-else class="text-sm font-medium text-gray-900 py-1">{{ tribo.nome }}</div>
            </td>
            <td class="px-4 py-2 whitespace-nowrap text-right text-sm font-medium">
              <div class="flex justify-end space-x-2">
                <template v-if="editando === tribo.id">
                  <button 
                    @click="salvarEdicao(tribo)"
            ...
            <td class="px-4 py-3">
              <div class="flex items-center space-x-2">
                ...
                <input 
                  ...
                  placeholder="Adicionar nova tribo..."
    ...
    <ConfirmModal 
      ...
      :message="`Tem certeza que deseja excluir a tribo '${itemParaExcluir?.nome}'?`"
...
export default {
  ...
  computed: {
    tribosOrdenadas() {
      const sorted = [...this.tribos].sort((a, b) => {
  ...
  methods: {
    async carregarDados() {
      this.loading = true
      try {
        const response = await axios.get('/api/tribos')
        this.tribos = response.data
      } catch (error) {
    ...
    iniciarEdicao(tribo) {
      this.editando = tribo.id
      this.nomeEditando = tribo.nome
      this.$nextTick(() => {
    ...
    async executarExclusao() {
      try {
        ...
        console.error('Erro ao excluir tribo:', error)
